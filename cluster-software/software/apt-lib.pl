...
print "<b>",&text('apt_install', "<tt>apt-get -y -f install $update</tt>"),"</b><p>\n";
print "<pre>";
$update = join(" ", map { quotemeta($_) } split(/\s+/, $update));
&additional_log('exec', undef, "apt-get -y -f install $update");
&open_execute_command(CMD, "apt-get -y -f install $update 2>&1 </dev/null", 1);
while(<CMD>) {
...
{
local (@rv, $pkg);
&open_execute_command(DUMP, "apt-cache search ".quotemeta($_[0]), 1, 1);
while(<DUMP>) {
	if (/^(\S+)\s*-\s*(.*)/) {
