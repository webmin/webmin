...
if (_smartctl_needs_type($out) || _smartctl_no_smart($out)) {
    my @types = qw(sat,auto sat scsi auto usbjmicron usbprolific usbcypress usbsunplus);
    my $best_out = $out;
    my $best_cmd = $cmd;
    foreach my $t (@types) {
        ...
        if (_smartctl_no_smart($try_out)) {
            my $perm_cmd = "smartctl -a -T permissive -d $t " . &quote_path($device) . " 2>&1";
            my $perm_out = &backquote_command($perm_cmd);
            if (!_smartctl_needs_type($perm_out) && !_smartctl_no_smart($perm_out)) {
                $out = $perm_out;
                $used_cmd = $perm_cmd;
                last;
            }
