...
if (($url =~ /\$VIRTUALSERVER_/ || $url =~ /\$\{VIRTUALSERVER_/) &&
	...
	if ($url && $config{'immediate'}) {
                if ($config{'window'}) {
                        ...
                        &ui_print_header(undef, $text{'index_title'}, "", undef, 1, 1);
                        print "<iframe width=100% style=\"height: 100vh; border:none\" frameBorder=0 src=\"$url\"></iframe>";
        ...
        &ui_print_header(undef, $text{'index_title'}, "", undef, 1, 1);
        if ($url) {
                        print "<iframe width=100% style=\"height: 100vh; border:none\" frameBorder=0 srcdoc=\"";
